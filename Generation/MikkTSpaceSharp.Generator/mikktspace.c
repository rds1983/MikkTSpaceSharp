			...
			if (!bFound)
			{
				...
				if (pIndices==NULL)
				{
					...
					for (k=0; k<iUniqueSubGroups; k++)
						free(pUniSubGroups[k].pTriMembers);
					free(pUniSubGroups);
					free(pTmpMembers);
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
	...
	for (i=0; i<iEntries; i++)
	{
		...
		const int f2 = pEdges[i].f;
		tbool bUnassigned_A;

		int i0_A, i1_A;
		int edgenum_A, edgenum_B=0;	// 0,1 or 2
		GetEdge(&i0_A, &i1_A, &edgenum_A, &piTriListIn[f2*3], i0, i1);	// resolve index ordering and edge_num
		bUnassigned_A = pTriInfos[f2].FaceNeighbors[edgenum_A] == -1 ? TTRUE : TFALSE;
	...
	while (t<iNrTrianglesIn && bStillFindingGoodOnes)
	{
		...
		else
		{
			...
			while (bJustADegenerate && iNextGoodTriangleSearchIndex<iTotTris)
			{
				...
				if (bIsGood2) bJustADegenerate=TFALSE;
